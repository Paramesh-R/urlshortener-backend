...
exports.createNewShortUrl = async (req, res, next) => {
    try {
        ...
        res
            .status(500)
            .send({ message: `Error creating short URL: ${error}` });
    }
}
...
exports.getUrlByUrlCode = async (req, res, next) => {
    try {
        ...
        if (!dataExists) return res.status(404).send({ message: "Invalid URL code" });
...
// READ ALL SHORT URL => POST: /api/url/
exports.readAllShortUrl = async (req, res, next) => {
    try {
        return res.status(200).send({ message: "readAllShortUrl" })
    } catch (error) {
        res.status(500).send({ message: "Error readAllShortUrl" })
    }
}
...
exports.updateShortUrl = async (req, res, next) => {
    try {
        ...
        return res.status(200).send({ message: "updateShortUrl" })

    } catch (error) {
        res.status(500).send({ message: "Error updateShortUrl" })
    }
}
